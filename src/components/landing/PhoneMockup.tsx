...
/** Mobile timeline mockup on the right of the Hero (presentational, built in code, no images). */
export default function PhoneMockup({ dict }: { dict: Dictionary["phone"] }) {
  return (
    <div className="relative flex justify-center lg:justify-end">
      ...
      {/* Floating trust chips (xl and up) */}
      <div className="absolute -left-2 top-10 z-20 hidden items-center gap-2.5 rounded-2xl border border-white/10 bg-vibe-card/85 px-4 py-3 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.9)] backdrop-blur-xl motion-safe:animate-float xl:flex">
        ...
        <div>
          <div className="text-[13px] font-semibold text-white">{dict.chipVoucher.title}</div>
          {/* The original design said "imported from your mailbox", which is not in the Fact Sheet (OCR parsing), so this states OCR instead */}
          <div className="text-[11.5px] text-white/45">{dict.chipVoucher.sub}</div>
        </div>
      </div>
      ...
      <div className="absolute -right-1 bottom-16 z-20 hidden items-center gap-2.5 rounded-2xl border border-white/10 bg-vibe-card/85 px-4 py-3 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.9)] backdrop-blur-xl motion-safe:animate-float-slow xl:flex">
        ...
        <div>
          <div className="text-[13px] font-semibold text-white">{dict.chipLocation.title}</div>
          <div className="text-[11.5px] text-white/45">{dict.chipLocation.sub}</div>
        </div>
      </div>
        ...
        <div className="relative overflow-hidden rounded-[33px] bg-[#0B0B12]">
          ...
          <div className="flex items-center gap-3 border-b border-white/[0.07] px-5 py-3.5">
            ...
            <div className="flex-1">
              <div className="text-[14px] font-semibold text-white">{dict.assistantName}</div>
              <div className="text-[11px] text-vibe-cyan">{dict.assistantStatus}</div>
            </div>
          ...
          <div className="flex flex-col gap-3 px-5 py-4">
            ...
            <div className="mt-1 rounded-[20px] border border-white/[0.08] bg-gradient-to-b from-white/[0.06] to-white/[0.02] p-4">
              <div className="mb-3.5 flex items-center justify-between">
                <span className="text-[12.5px] font-semibold text-white">{dict.dayLabel}</span>
              ...
              <div className="relative flex flex-col gap-3.5 pl-6">
                ...
                <TimelineItem
                  ...
                  title={dict.arrive.title}
                  time={dict.arrive.time}
                  sub={dict.arrive.sub}
                />
                <TimelineItem
                  dot="#8B7CFF"
                  icon={<BedDouble className="h-3.5 w-3.5 text-vibe-tint" />}
                  title={dict.checkin.title}
                  time={dict.checkin.time}
                  sub={
                    <>
                      {dict.checkin.sub}
                ...
                <TimelineItem
                  ...
                  title={dict.eats.title}
                  time={dict.eats.time}
                  sub={
                    <>
                      <Star className="h-3 w-3 text-vibe-cyan" /> {dict.eats.sub}
                    </>
                  }
                />
              </div>
            ...
            <div className="mt-1 flex items-center gap-2 self-start rounded-full border border-white/[0.07] bg-white/[0.04] px-3 py-2">
              ...
              <span className="text-[11px] text-white/45">{dict.typing}</span>
            </div>
          </div>

          {/* Input bar */}
          <div className="mx-5 mb-5 flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.04] px-4 py-2.5">
            <span className="flex-1 text-[12px] text-white/35">{dict.placeholder}</span>
